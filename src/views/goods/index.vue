<template>
  ...
  <span @click="$router.push('/cart')">购物车</span>
      ...
      <div class="btn-buy" @click="addF()">立刻购买</div>
    </div>
  ...
  <div class="product">
    ...
    <div class="num-box">
      <span>数量</span>
      <countBox v-model="countCh"></countBox>
    </div>
...
import loginConfirm from '@/minxins/loginConfirm'
export default {
  mixins: [loginConfirm],
  name: 'ProDetail',
  components: {
  ...
  methods: {
    ...
    async addCart () {
      ...
      const { data } = await AddCart(this.goodsId, this.countCh, this.detail.skuList[0].goods_sku_id)
  ...
  async created () {
    const { data: { detail } } = await getGoods(this.goodsId)
    this.detail = detail
